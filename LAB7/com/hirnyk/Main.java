    ...
    public static void main(String[] args) throws FileNotFoundException{
       ...
       while(true){
           print("\nФункції меню якими Ви можете користуватись\n"+
                   ...
                   "6 - сортування за іменем злочинця\n"+
                   "7 - видалення злочинця з списку\n"+
                   "8 - стандартна серіалізація\n"+
                   "9 - стандартна десеріалізація\n"+
                   "10 - пошук злочинця за іменем\n"+
           ...
           switch(command){
               ...
               case "5":
                   ...
                   XMLDecoder decoder = new XMLDecoder(
                                            new BufferedInputStream(
                                                new FileInputStream(SearchDirectory.search(".xml"))));
               ...
               case "6":
                   println("Сортування елементів по імені.");
                   ...
                   break;
               case "7":
                   print("Введіть порядковий номер злочинця для видалення (1-"+bandits.size()+"): ");
                   int index = write.nextInt();
                   bandits.remove(index-1);
                   print("Успішне видалення!");
                   break;
               case "8":
                   try {
                      FileOutputStream  outputStream = new FileOutputStream(SearchDirectory.search(".txt"));
               ...
               case "9":
                   FileInputStream fileInputStream;
                    try {
                        fileInputStream = new FileInputStream(SearchDirectory.search(".txt"));
                    } catch (FileNotFoundException ex) {
                            System.out.println("Something wrong....");
                            break;
                        }
               ...
               case "10":
                   print("Введіть ім'я для перевірки на наявінсть: ");
                   String name = write.nextLine();
                   boolean have = false;
                   for(Bandit i : bandits){
                       if (i.getPIB().equals(name)){
                           have = true;
                           break;
                       }
                   }
                   print("Дана особа в поліцейській картотеці - ");
                   if(have==true)
                       println("наявна");
                   else println("ненаявна");
                   break;
               case "exit":
